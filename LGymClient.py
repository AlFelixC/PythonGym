...
class LGymClient:
    ...
    def _precessingPerception(self, data):
        ...
        if self.IsCommand("perception",attributes) :
            if "parameters" in attributes:
                parametersAttr = attributes["parameters"]
                parameters = self._parseArray(";",parametersAttr,"float")
                return parameters, attributes["gameover"] == "True", attributes["destroyed"] == "True"
            else:
                return False, attributes["gameover"] == "True", attributes["destroyed"] == "True"
    ...
    def RecivePerception(self):
        data = self.lgconnectClinet.clientRecive()
        perceptions, gameover, destroyed = self._precessingPerception(data)
        if not perceptions:
            self.lgconnectClinet.clientSend("error=01&name=bad format\r\n")
        else:
            self.lgconnectClinet.clientSend("ok\r\n")
        return perceptions, gameover, destroyed
        ...
        if client.addCustomAgent(agent.Id(),agentName):
            ...
            if client.commandInit():
                ...
                while not finish:
                    ...
                    perception, gameover, destroyed = client.RecivePerception()
                    if gameover == True:
                        finish = True
                        win = True
                        if debug :
                        ...
                        action, fire=agent.Update(perception)
